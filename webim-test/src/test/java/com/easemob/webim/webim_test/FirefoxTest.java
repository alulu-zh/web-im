...
public class FirefoxTest extends WebIMTestBase {
	...
	private String username2; //default group memebers,used as add to group blacklist 
	private String password2;
	private String username3="sdb-add"; //add group members
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "creatGroups"  }, priority = 100)
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "sendGroupMessage" })
	 public void updateGroupInfo() throws Exception{
       ...
       if(element.getAttribute("class")=="webim-operations hide"){
        ...
        logger.info("if hide?{}",element.getAttribute("class"));
        xpath="//div[@class='webim-chatwindow ']/div[2]/div/ul[@tabindex='-1']/li/span[text()='修改群信息']";
        ele=findElement(driver, xpath,path);
        ele.click();        
        // check group list
        xpath="//div[@class='webim-dialog-body']/input[@placeholder='群组名']";
        String groupName="changed_group_name";
        element=findElement(driver,xpath,path);
        element.clear();
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "updateGroupInfo" })
	 public void addgroupmembers() throws Exception{
	        ...
	        logger.info("if hide?{}",element.getAttribute("class"));
	       // ele=driver.findElement(By.xpath("//div[@class='webim-chatwindow ']/div[2]/div/ul[@tabindex='-1']/li/span[text()='管理群成员']"));
	       // ele.click();
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "addgroupmembers" })
	 public void delgroupmembers() throws Exception{		
		 String path = getScreenshotPath(Thread.currentThread().getStackTrace()[1].getMethodName());
	        ...
	        //check if have add members
	    	sleep(3);
	    	if (!findGroupMembers(driver,groupID,path,members)){
	...
	public void addgroupblacklist() throws Exception{
	    String path = getScreenshotPath(Thread.currentThread().getStackTrace()[1].getMethodName());
	    List<String> members=new ArrayList<>();
	    members.add(username2);
	    ...
	    for(int k=0;k<members.size();k++){
		    for(int i=1;i<elements.size();i++){
	    		...
	    		if(StringUtils.contains(text,member)){
	    			int j=i+1;
	    			element.findElement(By.xpath("//li["+j+"]/div[@class='webim-operation-icon']/i[@class='webim-leftbar-icon font smaller  dib']")).click();	
	    			break;
	    		}
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "addgroupblacklist" })
	...
	public boolean getgroupblacklist() throws Exception{
	    String path = getScreenshotPath(Thread.currentThread().getStackTrace()[1].getMethodName());
	    sleep(5);
	    WebElement ele = findSpecialGroup(driver, groupID, path);
		//String groupID = ele.getAttribute("id");
        ...
        if(StringUtils.contains(ele.getText(), username2)){
        	return true;
        }
        else{
        	return false;
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "removegroupblacklist" })
	 public void readdgroupmembers() throws Exception{
		    String path = getScreenshotPath(Thread.currentThread().getStackTrace()[1].getMethodName());
		    List<String> members=new ArrayList<>();
		    members.add(username3);
		    logger.info("select first group");		    
	        ...
	        element.findElement(By.xpath("//li/span[text()='管理群成员']")).click();
	        sleep(5);	        
	        // check group list(should be a black list)
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "readdgroupmembers" })
	public void quitGroup(){
	    String path = getScreenshotPath(Thread.currentThread().getStackTrace()[1].getMethodName());
	    sleep(2);
	    driver2=new ChromeDriver();
	    super.login(driver2, username3, password3, path, true);
	    WebElement ele = findSpecialGroup(driver2, groupID, path);
		//String groupID = ele.getAttribute("id");
    	 // click setting button
        String xpath = RIGHT_SETTING_BUTTON_XPATH;
        ele = findElement(driver2, xpath,path);
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "quitGroup" })
	...
	@Test(enabled = true, groups = { "sanity_test" }, dependsOnMethods = { "destroyGroup" })
